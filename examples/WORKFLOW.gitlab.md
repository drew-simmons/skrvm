---
# Skrvm GitLab Issues Workflow Configuration (Production Checkout Workflow)
# Orchestrates coding agents on external repositories based on GitLab Issues.
# Ensure all sensitive API keys are stored in environment variables.

tracker:
  # Supported kinds: "linear", "jira", "github", "gitlab", or "memory"
  kind: "gitlab"

  # GitLab API endpoint: "https://gitlab.com"
  endpoint: "https://gitlab.com"

  # API key reference. Skrvm automatically resolves any string prefixed with '$' from process env.
  # Best Practice: export GITLAB_TOKEN="..." in your shell profile.
  api_key: "$GITLAB_TOKEN"

  # Project path / identifier slug (e.g. "my-namespace/my-target-project")
  project_slug: "my-namespace/my-target-project"

  # Filter tickets assigned to. Limit to issues assigned to this user, or leave blank.
  assignee: "my-gitlab-username"
...
hooks:
  ...
  after_create: "git clone git@gitlab.com:my-namespace/my-target-project.git . && git checkout -b {{ issue.branch_name }}"
...
resolve GitLab Issue **#{{ issue.identifier }}**.
